...
static void handleGamepadButton(Genesis& genesis, int port, SDL_GamepadButton btn, bool pressed) {
    ...
    //   LB→X, RB→Z, Start→Start, Back→Mode
    switch (btn) {
        ...
        case SDL_GAMEPAD_BUTTON_BACK:          genesis.setButton(port, 11, pressed); break; // Back → Mode
        default: break;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        if (maxFrames > 0 && genesis.getFrameCount() >= maxFrames) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            switch (event.type) {
                ...
                case SDL_EVENT_GAMEPAD_BUTTON_UP: {
                    ...
                    // Guide button toggles menu
                    if (btn == SDL_GAMEPAD_BUTTON_GUIDE && pressed) {
                        if (ui.isVisible()) {
                            ui.setVisible(false);
                            if (menuPauseEngaged) {
                                genesis.setPaused(pauseWasActiveBeforeMenu);
