...
static void loadConfig(int& windowScale, ScalingMode& scalingMode, AspectRatio& aspectRatio,
                        ...
                        bool& autoLoadState, ShaderMode& shaderMode) {
    ...
    while (std::getline(in, line)) {
        ...
        if (section == "video") {
            if (key == "window_scale") {
                ...
                if (val == "crt") shaderMode = ShaderMode::CRT;
                else shaderMode = ShaderMode::None;
            } else if (key == "vsync") {
...
static void saveConfig(int windowScale, ScalingMode scalingMode, AspectRatio aspectRatio,
    ...
    out << "shader_mode = " << (shaderMode == ShaderMode::CRT ? "crt" : "none") << "\n";
...
int main(int argc, char* argv[]) {
    ...
    loadConfig(windowScale, scalingMode, aspectRatio, vsyncEnabled,
               ...
               fullscreenMode, fullscreenWidth, fullscreenHeight, fullscreenRefreshHz, autoLoadState, shaderMode);
    ...
    gameRenderer.setShaderMode(shaderMode);
    ui.setCRTShader(shaderMode == ShaderMode::CRT);
#ifndef USE_VULKAN
    ...
    auto applyAction = [&](const AppUI::Action& action) {
        switch (action.type) {
            ...
            case AppUI::ActionType::SetShaderMode:
                shaderMode = action.intValue ? ShaderMode::CRT : ShaderMode::None;
                gameRenderer.setShaderMode(shaderMode);
                gameRenderer.setCRTParams(ui.crtCurvature(), ui.crtScanline(),
                                          ui.crtBloom(), ui.crtVignette(), ui.crtMaskType());
                break;
