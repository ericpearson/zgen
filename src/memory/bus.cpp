...
void Bus::write8(u32 addr, u8 val, int partialCycles) {
    ...
    if (addr >= 0xFF0000) {
        ...
        if (ramWriteWatchMatches(addr)) {
            std::fprintf(stderr,
                         "[RAMW8] addr=%06X val=%02X pc=%06X ln=%d\n",
                         addr,
                         val,
    ...
    if (addr >= 0xC00000 && addr < 0xC00020) {
        ...
        if (fifoTimingLogEnabled()) {
            std::fprintf(stderr, "[VDP-W8] addr=%06X val=%02X busPartialCyc=%d m68kCyc=%d memOff=%d\n",
...
void Bus::write16(u32 addr, u16 val, int partialCycles) {
    ...
    if (addr >= 0xC00000 && addr < 0xC00020) {
        ...
        if (m68k) m68k->applyRefreshFreeAccess();
        if (fifoTimingLogEnabled()) {
            std::fprintf(stderr, "[VDP-W16] addr=%06X val=%04X busPartialCyc=%d m68kCyc=%d memOff=%d\n",
                         addr, val, partialCycles,
                         m68k ? m68k->state.cycles : -1,
                         m68k ? m68k->getMemoryTimingOffset() : -1);
        }
    ...
    if (addr >= 0xFF0000) {
        ...
        if (ramWriteWatchMatches(addr) || ramWriteWatchMatches(addr + 1)) {
            std::fprintf(stderr,
                         "[RAMW16] addr=%06X val=%04X pc=%06X ln=%d\n",
                         addr,
                         val,
                         m68k ? m68k->getPC() & 0xFFFFFF : 0,
                         vdp ? vdp->getScanline() : -1);
...
u8 Bus::z80Read(u16 addr) {
    ...
    // TODO: Z80 68K-bus access should stall the M68K for bus arbitration
    // (~7-8 master clocks per access for the bus handshake).
    if (addr >= 0x8000) {
        z80BankAccessCount++;
