    ...
    if (addr >= 0xC00000 && addr < 0xC00020) {
        ...
        switch (reg) {
            ...
            case 2: case 3: { // Control port (status)
                int partial = m68k ? m68k->state.cycles : 0;
                u16 s = vdp->readControl(partial);
...
u16 Bus::read16(u32 addr) {
    ...
    if (addr >= 0xC00000 && addr < 0xC00020) {
        ...
        switch (reg) {
            ...
            case 2: case 3:  // Control port (status)
                ...
                return vdp->readControl(m68k ? m68k->state.cycles : 0);
            case 4: case 5: case 6: case 7:  // HV counter
    ...
    if (addr >= 0xA00000 && addr < 0xA10000) {
        ...
        if (addr >= 0xA04000 && addr < 0xA04004) {
            const u8 status = ym2612 ? ym2612->readStatus() : 0;
            return static_cast<u16>((status << 8) | status);
        }
    }
...
u8 Bus::z80Read(u16 addr) {
    ...
    // YM2612: $4000-$5FFF
    if (addr < 0x6000) {
        if (ym2612) return ym2612->readStatus();
        return 0;
    }
