...
void VDP::drainPendingFIFOForSnapshot() {
    // Drain all pending FIFO entries at the current line's early external
    // slots, then latch vsramSnapshot. This ensures VSRAM writes from the
    // previous line's H-int handler (which couldn't drain because no
    // external slots remained) are applied before the snapshot is taken.
    if (fifoCount > 0 && scanline > 0 && scanline <= activeHeight) {
        const VDPSlotTable& table = currentSlotTable();
        int saveCycle = lineCycles;
        int saveSlot = currentSlotIndex;
        int s = (table.count > 0) ? table.nextExternalSlot[0] : table.count;
        while (fifoCount > 0 && s < table.count) {
            lineCycles = firstM68kCycleForSlot(table, s);
            currentSlotIndex = s;
            processExternalSlot();
            if (fifoCount == 0) break;
            s = (s + 1 < table.count) ? table.nextExternalSlot[s + 1] : table.count;
        }
        lineCycles = saveCycle;
        currentSlotIndex = saveSlot;
    }
    vsramSnapshot[0] = vsram[0];
    vsramSnapshot[1] = vsram[1];
}
...
void VDP::beginScanlineCommon() {
    ...
    // Drain pending FIFO before latching, then latch.
    drainPendingFIFOForSnapshot();
    vsramSnapshot[0] = vsram[0];
    vsramSnapshot[1] = vsram[1];
    ...
    // - Scanline 0: PREPARING line equivalent — decrement once (not reload).
    //   The VDP has a transitional "PREPARING" state at vcounter 0x1FF that
    //   decrements the counter before active display begins. This consumes
    //   one tick, fixing Panorama Cotton's HUD (handler A6 pointer alignment).
    if (scanline == 0) {
        if (hblankCounter == 0) {
...
int VDP::nextCpuEventLineCycle() const {
    ...
    if (!hblankCounterDecremented_ && scanline < activeHeight &&
        hblankStartCycle_ > lineCycles && hblankStartCycle_ < nextCycle) {
        nextCycle = hblankStartCycle_;
    }
