...
void VDP::renderBackground(int line, int layer, int startX, int endX, u16* lineBuffer, u8* rankBuffer) {
    ...
    // Full-screen V-scroll uses the line-start snapshot so late-line renders
    // (e.g. endScanlineTimed flush) see the value that was in VSRAM when the
    // line began, not a value written by a later H-int or V-int handler.
