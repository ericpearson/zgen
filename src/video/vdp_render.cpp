...
void VDP::renderBackground(int line, int layer, int startX, int endX, u16* lineBuffer, u8* rankBuffer) {
    ...
    // Full-screen V-scroll: use the latched snapshot, not live VSRAM.
    // The snapshot is taken at cycle ~112, after the H-int handler has
    // had time to write the current line's scroll value.  Using live
    // vsram[] would read the NEXT line's value if the handler pre-apply
    // in writeData() has already fired (breaks Top Gear 2 sky rendering).
    int fullVScroll = vsramSnapshot[layer] & 0x03FF;

    {
        static const bool logScroll = []() {
            const char* e = std::getenv("GENESIS_LOG_SCROLL");
            return e && e[0] != '0';
        }();
        if (logScroll && layer == 0 && startX == 0) {
            std::fprintf(stderr, "[SCROLL] ln=%d vsA=%d vsB=%d hsA=%d hsB=%d\n",
                         line, vsram[0] & 0x3FF, vsram[1] & 0x3FF,
                         hscrollSnapshot[0] & 0x3FF, hscrollSnapshot[1] & 0x3FF);
        }
    }
...
void VDP::renderSprites(int line, int startX, int endX, u16* lineBuffer, u8* rankBuffer) {
    ...
    // Sprite X=0 masking (matches GPGX's spr_ovr).  If the PREVIOUS line
    ...
    for (int i = 0; i < spriteCount; i++) {
        ...
        if (rawX != 0) {
            ...
            masked = true;
            // Don't break — continue loop to match GPGX behavior where
            // the loop runs to completion and spr_ovr is cleared.
        }
    ...
    // non-zero-X sprite.  Matches GPGX: spr_ovr is set when xpos != 0,
    // cleared at end of line (unless dot overflow, which we don't track).
