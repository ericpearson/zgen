...
void Genesis::runScanline() {
    ...
    if (!vdp.vblankPending() && vdp.hblankPending()) {
        if (logHintTiming) {
...
static constexpr u32 SAVE_VERSION = 15;
...
bool Genesis::loadStateFromFile(const std::string& path) {
    ...
    if (logStateLoad) {
        std::fprintf(stderr,
                     "[STATE-LOAD] frame=%d scanline=%d lineCyc=%d pc=%06X sr=%04X debt=%d lineAccum=%d fe10=%02X fe11=%02X ffd8w=%04X ffd8=%02X ffd9=%02X dispEn=%d dispLat=%d hint=%d vint=%d slot=%d\n",
                     frameCount,
                     vdp.scanline,
                     vdp.lineCycles,
                     m68k.getPC(),
