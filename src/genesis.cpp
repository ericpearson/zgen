...
inline bool cottonRamLogShouldLog(int frame) {
    static int enabled = -1;
    static int frameFirst = 0;
    static int frameLast = 0;
    if (enabled < 0) {
        const char* env = std::getenv("GENESIS_LOG_COTTON_RAM");
        enabled = (env && env[0] != '0') ? 1 : 0;
        frameFirst = envIntOrDefault("GENESIS_LOG_COTTON_RAM_FRAME_FIRST", 0);
        frameLast = envIntOrDefault("GENESIS_LOG_COTTON_RAM_FRAME_LAST", 0);
    }
    if (!enabled) {
        return false;
    }
...
inline bool cottonRamScanlineLogShouldLog(int frame, int scanline) {
    static int enabled = -1;
    static int frameFirst = 0;
    static int frameLast = 0;
    ...
    if (enabled < 0) {
        const char* env = std::getenv("GENESIS_LOG_COTTON_RAM_SCANLINES");
        enabled = (env && env[0] != '0') ? 1 : 0;
        frameFirst = envIntOrDefault("GENESIS_LOG_COTTON_RAM_FRAME_FIRST", 0);
        frameLast = envIntOrDefault("GENESIS_LOG_COTTON_RAM_FRAME_LAST", 0);
        lineFirst = envIntOrDefault("GENESIS_LOG_COTTON_RAM_LINE_FIRST", 0);
        lineLast = envIntOrDefault("GENESIS_LOG_COTTON_RAM_LINE_LAST", 0);
    }
    if (!enabled) {
        return false;
    }
    if (frameFirst || frameLast) {
        const int last = frameLast ? frameLast : frameFirst;
        if (frame < frameFirst || frame > last) {
            return false;
        }
    }
    if (lineFirst || lineLast) {
        const int last = lineLast ? lineLast : lineFirst;
        if (scanline < lineFirst || scanline > last) {
            return false;
        }
    }
    return true;
...
inline void logCottonRamSlice(const char* phase, int frame, const Bus& bus) {
    if (!cottonRamLogShouldLog(frame)) {
        return;
    }
    ...
    auto ramByte = [&](u32 addr) -> u8 {
        return ram[addr & 0xFFFF];
    };
    std::fprintf(stderr,
                 "[COTTON-RAM] impl=ours phase=%s frame=%d ff8e04=%04X ff8e05=%02X ff9fa4=%04X ff9fba=%04X ff95a8=%04X ff9604=%04X fff463=%02X fff466=%02X\n",
                 phase,
                 frame,
                 ramWord(0xFF8E04),
                 ramByte(0xFF8E05),
                 ...
                 ramByte(0xFFF463),
                 ramByte(0xFFF466));
    for (u32 addr = 0xFF95B0; addr <= 0xFF9600; addr += 0x10) {
        std::fprintf(stderr,
                     "[COTTON-RAM] impl=ours phase=%s frame=%d addr=%06X %04X %04X %04X %04X %04X %04X %04X %04X\n",
                     phase,
                     frame,
                     addr,
                     ramWord(addr + 0x00),
...
inline void logCottonRamScanline(const char* phase,
                                 int frame,
                                 int scanline,
                                 const Bus& bus,
                                 const VDP& vdp) {
    if (!cottonRamScanlineLogShouldLog(frame, scanline)) {
        return;
    }
    const u8* ram = bus.getRam();
    auto ramWord = [&](u32 addr) -> u16 {
    ...
    const u16* vsram = vdp.getVSRAM();
    const u16* vscrollLatch = vdp.getVScrollLatch();
    std::fprintf(stderr,
                 "[COTTON-RAM-LN] impl=ours phase=%s frame=%d line=%d ff8e04=%04X ff8e05=%02X ff95bc=%04X ff9fa4=%04X ff9fba=%04X fff466=%02X vsrawA=%03X vsrawB=%03X vslatchA=%03X vslatchB=%03X\n",
                 phase,
                 frame,
                 scanline,
                 ramWord(0xFF8E04),
...
void Genesis::clockYM(int m68kCycles) {
    ...
    // titles use ~3 kHz) — without filtering, the staircase waveform
    // produces harsh high-frequency aliasing artifacts.
    // Disable with GENESIS_YM_LPF=0 for brighter unfiltered sound.
    static const bool lpfEnabled = []() {
        const char* e = std::getenv("GENESIS_YM_LPF");
        return !(e && e[0] == '0');  // enabled by default
    }();
...
void Genesis::clockZ80Master(int masterCycles) {
    ...
    // so consumed = (initial - current) is non-negative.
    if (z80BurstInitialDebt_ == INT_MIN) {
        z80BurstInitialDebt_ = z80CycleDebt;
    }

    if (bus.z80Reset || bus.z80BusRequested) {
        // Z80 is halted but the Z80 clock still ticks; advance the bus's
...
void Genesis::syncYMBeforeWrite() {
    // Called from bus.cpp when Z80 writes to YM2612 registers.
    // Advance YM (generating samples with the OLD register values) up to
    // the Z80's CURRENT position within the burst, so consecutive writes
    // within one burst are correctly serialized in time.
    //
    // The previous implementation drained the entire burstCycles on the
    // FIRST sync call, then did nothing for subsequent calls. Empirically,
    // ~50% of Z80→YM writes in PCM-streaming games (Codemasters titles)
    // came in clusters where only the first write actually advanced the YM,
    // and the rest stacked up at the same effective YM time. The fix tracks
    // Z80 progress (z80BurstInitialDebt_ - z80CycleDebt) and drains only
    // the M68K-cycle delta since the last sync.
    if (ymBurstTotalCycles_ <= 0) {
        return; // not inside a Z80 burst
    }
    int z80Consumed = z80BurstInitialDebt_ - z80CycleDebt;
    if (z80Consumed < 0) z80Consumed = 0;
    // Z80 cycles → master clocks (×Z80_DIVIDER) → M68K cycles (÷M68K_DIVIDER)
    int m68kEquivalent = (z80Consumed * Z80_DIVIDER) / M68K_DIVIDER;
    if (m68kEquivalent > ymBurstTotalCycles_) {
        m68kEquivalent = ymBurstTotalCycles_;
    }
...
void Genesis::runFrame() {
    ...
    for (int line = 0; line < getScanlinesPerFrame(); line++) {
        ...
        runScanline();
        if (!pendingFrameBoundaryVdpDumpPath_.empty() && line == vdp.activeHeight) {
            if (!dumpVdpStateBin(pendingFrameBoundaryVdpDumpPath_,
                                 pendingFrameBoundaryVdpDumpIncludeM68kState_)) {
                std::fprintf(stderr,
                             "failed to dump frame-boundary VDP state to %s\n",
                             pendingFrameBoundaryVdpDumpPath_.c_str());
            }
            pendingFrameBoundaryVdpDumpPath_.clear();
        }
    }
...

    {
        static const bool logStalls = []() {
            const char* e = std::getenv("GENESIS_LOG_STALLS");
            return e && e[0] != '0';
        }();
        if (logStalls) {
            std::fprintf(stderr, "[STALLS] frame=%d dma=%llu fifo=%llu total=%llu\n",
                         frameCount,
                         static_cast<unsigned long long>(stallCyclesDMA_),
                         static_cast<unsigned long long>(stallCyclesFIFO_),
                         static_cast<unsigned long long>(stallCyclesDMA_ + stallCyclesFIFO_));
        }
    }
...
bool Genesis::dumpVdpStateBin(const std::string& path, bool includeM68kState) const {
    ...
    for (int i = 0; i < 64; i++) {
        const std::uint16_t packed = packCram9Bit(vdp.getCRAM()[i]);
        bin.write(reinterpret_cast<const char*>(&packed), sizeof(packed));
    }
    bin.write(reinterpret_cast<const char*>(vdp.getVSRAM()),
              static_cast<std::streamsize>(sizeof(std::uint16_t) * 40));
    ...
    if (includeM68kState) {
        const auto& m68kState = m68k.getState();
        bin.write(reinterpret_cast<const char*>(m68kState.d),
                  static_cast<std::streamsize>(sizeof(std::uint32_t) * 8));
        bin.write(reinterpret_cast<const char*>(m68kState.a),
                  static_cast<std::streamsize>(sizeof(std::uint32_t) * 8));
...
void Genesis::runScanline() {
    ...
    while (m68kCyclesRemaining > 0) {
        ...
        // Z80→YM2612 sync: set pending YM cycles before Z80 runs.
        // When Z80 writes to YM registers, syncYMBeforeWrite() drains
        // YM up to the Z80's CURRENT progress (not the full burst), so
        // consecutive writes within one burst are serialized in time.
        // After Z80 finishes, remaining pending cycles generate samples
        // with the FINAL register state.
        ymPendingCycles_ = burstCycles;
        ymBurstTotalCycles_ = burstCycles;
        ymBurstDrained_ = 0;
        z80BurstInitialDebt_ = INT_MIN; // sentinel: clockZ80 records actual value
        if (profile) {
            auto z80Start = ProfileClock::now();
        ...
        // Generate remaining YM samples (after last Z80→YM write, if any)
        if (ymPendingCycles_ > 0) {
            if (profile) {
                auto ymStart = ProfileClock::now();
                clockYM(ymPendingCycles_);
    ...
    // Phase 4: Refresh penalty is now charged inline via M68K::advanceClock
    // (which adds the penalty to state.cycles directly), so the per-line
    // pendingBudgetPenalty_ drain is gone. The previous pathway accumulated
    // refresh delays at IO sites and deducted them here at end-of-line; that
    // entire pathway has been replaced by per-mclk-advance refresh charging
    // in src/cpu/m68k.h:99-115.
...
bool Genesis::canonicalizeLoadedBoundaryState() {
    ...
    vdp.boundaryVscrollLatchValid_ = false;
...
bool Genesis::saveState(int slot, const u32* screenshotBuffer) {
    ...
    writeArr(f, vdp.vscrollLatch, 2);
    writeArr(f, vdp.visibleLineVscroll, 2);
    ...
    writeArr(f, vdp.boundaryVscrollLatch_, 2);
    writeVal(f, vdp.boundaryVscrollLatchValid_);
...
bool Genesis::loadStateFromFile(const std::string& path) {
    ...
    // Phase 1 scaffolding: wall-clock shadows masterCycles_; reset on load.
    m68k.wallClockMclks_ = m68k.masterCycles_;
    ...
    if (version >= 18) {
        readArr(f, vdp.vscrollLatch, 2);
        readArr(f, vdp.visibleLineVscroll, 2);
    } else {
        vdp.vscrollLatch[0] = vdp.vsram[0];
        vdp.vscrollLatch[1] = vdp.vsram[1];
        vdp.visibleLineVscroll[0] = vdp.vsram[0];
        vdp.visibleLineVscroll[1] = vdp.vsram[1];
    }
    ...
    if (version >= 17) {
        ...
        // Version 3-16: VDPFifoEntry lacked enqueueScanline/enqueueLineCycle.
        // Old struct: u16 value, u16 address, u8 code, bool byteWrite, bool lowByte
        // = 8 bytes with alignment. New struct = 16 bytes. Read old layout manually.
        struct VDPFifoEntryV16 {
            u16 value = 0;
            u16 address = 0;
            u8 code = 0;
            bool byteWrite = false;
            bool lowByte = false;
            u8 _pad = 0; // compiler pads to 8 bytes
        };
        static_assert(sizeof(VDPFifoEntryV16) == 8, "V16 FIFO entry must be 8 bytes");
        VDPFifoEntryV16 oldFifo[VDP::FIFO_SIZE];
        readArr(f, oldFifo, VDP::FIFO_SIZE);
        for (int i = 0; i < VDP::FIFO_SIZE; i++) {
            vdp.fifo[i] = {};
            vdp.fifo[i].value = oldFifo[i].value;
            vdp.fifo[i].address = oldFifo[i].address;
            vdp.fifo[i].code = oldFifo[i].code;
            vdp.fifo[i].byteWrite = oldFifo[i].byteWrite;
            vdp.fifo[i].lowByte = oldFifo[i].lowByte;
            // enqueueScanline/enqueueLineCycle left 0 — treated as "always ready"
        }
    } else {
    ...
    u16 loadedBoundaryVscrollLatch[2] = {};
    bool loadedBoundaryVscrollLatchValid = false;
    ...
    if (version >= 8) {
        ...
        if (version >= 19) {
            readArr(f, loadedBoundaryVscrollLatch, 2);
            readVal(f, loadedBoundaryVscrollLatchValid);
        }
    } else {
    ...
    vdp.hintAssertCycle_ = activeTimingTable.hintAssertM68kCycle;
    ...
    if (version >= 13) {
        ...
        vdp.hblankCounterDecremented_ =
            (vdp.scanline < vdp.activeHeight) && (vdp.lineCycles >= vdp.hintAssertCycle_);
        vdp.refreshVBlankIRQState();
    }
    if (version >= 19) {
        std::memcpy(vdp.boundaryVscrollLatch_,
                    loadedBoundaryVscrollLatch,
                    sizeof(vdp.boundaryVscrollLatch_));
        vdp.boundaryVscrollLatchValid_ = loadedBoundaryVscrollLatchValid;
    } else {
        vdp.boundaryVscrollLatch_[0] = vdp.vscrollLatch[0];
        vdp.boundaryVscrollLatch_[1] = vdp.vscrollLatch[1];
        vdp.boundaryVscrollLatchValid_ = false;
    }
