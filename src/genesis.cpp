...
Genesis::Genesis() : frameCount(0), paused(false),
    ...
    // Connect components
    m68k.connectBus(&bus);
    m68k.refreshEnabled_ = true;  // Enable bus refresh for real system
    z80.setBus(&bus);
    vdp.connectBus(&bus);
    bus.connectCPU(&m68k);
    bus.connectVDP(&vdp);
...
void Genesis::runScanline() {
    ...
    if (carryInCycles > 0) {
        ...
        m68k.advanceMasterClocks(carryInCycles);
    }
