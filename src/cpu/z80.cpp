...
int Z80::execute() {
    ...
    // Check /INT line (level-triggered) — service if iff1 is set and not
    // immediately after EI (real Z80 delays interrupt enable by 1 instruction).
    if (state.irqLine && state.iff1 && !state.afterEi) {
        state.halted = false;
        state.iff1 = state.iff2 = false;
        push16(state.pc);
        switch (state.im) {
        ...
        case 0:
            switch (z) {
                ...
                case 2:
                    ...
                    // LD (nn),HL / LD HL,(nn) = 16; LD (nn),A / LD A,(nn) = 13; rest = 7
                    if (y >= 6) state.cycles += 13;
                    else if (y >= 4) state.cycles += 16;
                    else state.cycles += 7;
                    break;
                case 3:
                    if (q == 0) { // INC rp
...
void Z80::executeCB(u8 opcode) {
    ...
    // BIT b,(HL) = 12; other CB (HL) ops = 15; register ops = 8
    if (x == 1 && z == 6) state.cycles += 12;
    else state.cycles += (z == 6) ? 15 : 8;
}
...
void Z80::executeDD(u8 opcode) {
    ...
    state.ix = getHL();
    setHL(savedHL);
    state.cycles += 4;  // DD prefix penalty
}
...
void Z80::executeFD(u8 opcode) {
    ...
    state.iy = getHL();
    setHL(savedHL);
    state.cycles += 4;  // FD prefix penalty
}
...
void Z80::executeDDCB(u8 opcode, s8 disp) {
    ...
    state.cycles += (x == 1) ? 20 : 23;  // BIT = 20, others = 23
}
