...
void M68K::processInterrupt() {
    ...
    if (level > mask || level == 7) {
        ...
        int vector = 24 + level;
        exception(vector);
        // Genesis uses autovector interrupts via !VPA, which synchronizes
        // the acknowledge cycle to the E clock (1/10 of 68K clock).
        // Real hardware takes 49-59 total cycles depending on E-clock
        // phase. Use 54 (the approximate average) as a fixed value,
        // matching the jgenesis approach.  Variable E-clock timing
        // (GPGX-style) is more precise but requires all other timing
        // to be equally accurate to avoid regressions.
        // Reference: MC68000 User Manual appendix B, GPGX m68kcpu.h,
        // BlastEm genesis.c, jgenesis issue #419.
        state.cycles += 20;
        setIntMask(level);
    } else {
