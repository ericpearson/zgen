...
int M68K::applyRefresh() {
    ...
    refreshCounter_ = static_cast<u32>(total % kRefreshInterval);
    if (penalties <= 0) return 0;
    int added = kRefreshDelay * penalties;
    totalCycles_ += added;
    masterCycles_ += static_cast<u64>(added) * 7;
    return added;
}
...
int M68K::applyRefreshFreeAccess() {
    ...
    // Penalties for time before the current 4-MCLK access
    u64 penaltyElapsed = elapsed - 4 * 7;
    u64 total = static_cast<u64>(refreshCounter_) + penaltyElapsed;
    int penalties = static_cast<int>(total / kRefreshInterval);
    u32 counter = static_cast<u32>(total % kRefreshInterval);
    int added = 0;
    if (penalties > 0) {
        added = kRefreshDelay * penalties;
        totalCycles_ += added;
        masterCycles_ += static_cast<u64>(added) * 7;
    }
    // Free-access portion: advance counter without penalty
    counter = static_cast<u32>(
        (static_cast<u64>(counter) + 4 * 7) % kRefreshInterval);
    refreshCounter_ = counter;
    refreshLastSync_ = masterCycles_;
    return added;
}
...
void M68K::writeEA(int mode, int reg, int size, u32 val) {
    switch (mode) {
        ...
        case 1: // An
            ...
            return;
        default: {
            const int partialCycles = state.cycles + eaWriteCycles(mode, reg, size);
            u32 addr = getEA(mode, reg, size);
            switch (size) {
