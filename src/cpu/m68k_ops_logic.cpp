...
void M68K::op_bchg() {
    ...
    // PRM: dynamic Dn=8, static Dn=12, dynamic mem=8+EA, static mem=12+EA
    bool isStatic = !(state.ir & 0x100);
    if (mode == 0) {
        bitNum &= 31;
        setFlag(FLAG_Z, (state.d[reg] & (1 << bitNum)) == 0);
        state.d[reg] ^= (1 << bitNum);
        state.cycles += isStatic ? 12 : 8;
    } else {
        bitNum &= 7;
...
void M68K::op_bclr() {
    ...
    // PRM: dynamic Dn=10, static Dn=14, dynamic mem=8+EA, static mem=12+EA
    bool isStatic = !(state.ir & 0x100);
    if (mode == 0) {
        bitNum &= 31;
        setFlag(FLAG_Z, (state.d[reg] & (1 << bitNum)) == 0);
        state.d[reg] &= ~(1 << bitNum);
        state.cycles += isStatic ? 14 : 10;
    } else {
        bitNum &= 7;
        u32 addr = getEA(mode, reg, SIZE_BYTE);
        u8 val = readMemSized(addr, SIZE_BYTE);
