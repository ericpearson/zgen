...
void M68K::op_eor() {
    ...
    if (mode == 0) {
        ...
        u32 dst = readMemSized(addr, size);
        result = src ^ dst;
        writeMemSizedTimed(addr, size, result, state.cycles + eaWriteCycles(mode, dstReg, size));
    }
...
void M68K::op_bchg() {
    ...
    if (mode == 0) {
        ...
        setFlag(FLAG_Z, (val & (1 << bitNum)) == 0);
        writeMemSizedTimed(addr, SIZE_BYTE, val ^ (1 << bitNum), state.cycles + eaWriteCycles(mode, reg, SIZE_BYTE));
