...
void M68K::op_eor() {
    ...
    if (mode == 0) {
        ...
        const int readAccessCycles = state.cycles + eaReadAccessCycles(mode, dstReg, size);
        u32 dst = readMemSizedTimed(addr, size, readAccessCycles);
        result = src ^ dst;
        writeMemSizedTimed(addr, size, result, readAccessCycles + ((size == SIZE_LONG) ? 8 : 4));
    }
    ...
    if (mode == 0) {
        ...
        chargeCycles((size == SIZE_LONG) ? 12 : 8);
        chargeCycles(eaCycles(mode, dstReg, size));
    }
}
...
void M68K::op_tas() {
    ...
    chargeCycles((mode == 0) ? 4 : 10 + eaCycles(mode, reg, SIZE_BYTE));
}
...
void M68K::op_btst() {
    ...
    if (mode == 0) {
        ...
        chargeCycles((isStatic ? 10 : 6) - (isStatic ? 4 : 0));
    } else {
        bitNum &= 7;
        u8 val = readEA(mode, reg, SIZE_BYTE);
        setFlag(FLAG_Z, (val & (1 << bitNum)) == 0);
        chargeCycles((isStatic ? 8 : 4) - (isStatic ? 4 : 0) + eaCycles(mode, reg, SIZE_BYTE));
    }
}
...
void M68K::op_bchg() {
    ...
    if (mode == 0) {
        ...
        if (isStatic) {
            chargeCycles(((bitNum < 16) ? 10 : 12) - 4);
        } else {
            chargeCycles((bitNum < 16) ? 6 : 8);
        }
    } else {
        bitNum &= 7;
        u32 addr = getEA(mode, reg, SIZE_BYTE);
        const int readAccessCycles = state.cycles + eaReadAccessCycles(mode, reg, SIZE_BYTE);
        u8 val = readMemSizedTimed(addr, SIZE_BYTE, readAccessCycles);
        setFlag(FLAG_Z, (val & (1 << bitNum)) == 0);
        writeMemSizedTimed(addr, SIZE_BYTE, val ^ (1 << bitNum), readAccessCycles + 4);
        chargeCycles((isStatic ? 12 : 8) - (isStatic ? 4 : 0) + eaCycles(mode, reg, SIZE_BYTE));
    }
}
...
void M68K::op_bclr() {
    ...
    if (mode == 0) {
        ...
        state.d[reg] &= ~(1 << bitNum);
        if (isStatic) {
            chargeCycles(((bitNum < 16) ? 12 : 14) - 4);
        } else {
            chargeCycles((bitNum < 16) ? 8 : 10);
        }
    } else {
        bitNum &= 7;
        u32 addr = getEA(mode, reg, SIZE_BYTE);
