...
void M68K::op_add() {
    ...
    if (toReg) {
        ...
        u32 result = doAdd(src, dst, size);
        writeMemSizedTimed(addr, size, result, state.cycles + eaWriteCycles(mode, srcReg, size));
...
void M68K::op_addi() {
    ...
    if (mode == 0) {
        ...
        u32 dst = readMemSized(addr, size);
        u32 result = doAdd(imm, dst, size);
        writeMemSizedTimed(addr, size, result, state.cycles + eaWriteCycles(mode, reg, size));
    }
...
void M68K::op_addq() {
    ...
    if (mode == 0) {
        ...
        state.cycles += (size == SIZE_LONG) ? 24 : 12;
    }
}
...
void M68K::op_subq() {
    ...
    if (mode == 0) {
        ...
        state.cycles += (size == SIZE_LONG) ? 40 : 20;
    }
}
...
void M68K::op_clr() {
    ...
    if (mode == 0) {
        ...
        u32 addr = getEA(mode, reg, size);
        readMemSized(addr, size);  // dummy read (hardware behavior)
        writeMemSizedTimed(addr, size, 0, state.cycles + eaWriteCycles(mode, reg, size));
        state.cycles += (size == SIZE_LONG) ? 12 : 8;
        state.cycles += eaCycles(mode, reg, size);
    }
...
void M68K::op_nbcd() {
    ...
    if (mode == 0) {
        ...
        writeMemSizedTimed(addr, SIZE_BYTE, result, state.cycles + eaWriteCycles(mode, reg, SIZE_BYTE));
    }
