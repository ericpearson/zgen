...
void M68K::op_scc() {
    ...
    if (mode == 0) {
        state.cycles += result ? 6 : 4;
    } else {
        state.cycles += 8 + eaCycles(mode, reg, SIZE_BYTE);
    }
}
...
void M68K::op_jmp() {
    ...
    switch (mode) {
        case 2: state.cycles += 8; break;   // (An)
        case 5: state.cycles += 10; break;  // d(An)
        case 6: state.cycles += 14; break;  // d(An,Xi)
        case 7:
            switch (reg) {
                case 0: state.cycles += 10; break;  // xxx.W
                case 1: state.cycles += 12; break;  // xxx.L
                case 2: state.cycles += 10; break;  // d(PC)
                case 3: state.cycles += 14; break;  // d(PC,Xi)
                default: state.cycles += 8; break;
            }
            break;
        default: state.cycles += 8; break;
    }
...
void M68K::op_jsr() {
    ...
    switch (mode) {
        case 2: state.cycles += 16; break;  // (An)
        case 5: state.cycles += 18; break;  // d(An)
        case 6: state.cycles += 22; break;  // d(An,Xi)
        case 7:
            switch (reg) {
                case 0: state.cycles += 18; break;  // xxx.W
                case 1: state.cycles += 20; break;  // xxx.L
                case 2: state.cycles += 18; break;  // d(PC)
                case 3: state.cycles += 22; break;  // d(PC,Xi)
                default: state.cycles += 16; break;
            }
            break;
        default: state.cycles += 16; break;
    }
...
void M68K::op_trap() {
    ...
    exception(32 + vector);
    state.cycles += 4;  // Total = 38 per PRM (exception() already adds 34)
}
