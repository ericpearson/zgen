...
void M68K::op_asl_asr_reg() {
    ...
    state.cycles += ((size == SIZE_LONG) ? 8 : 6) + 2 * count;
}
...
void M68K::op_asl_asr_mem() {
    ...
    u16 val = readMemSized(addr, SIZE_WORD);
    bool lastBit;
    bool overflow = false;
    ...
    writeMemSizedTimed(addr, SIZE_WORD, val, state.cycles + eaWriteCycles(mode, reg, SIZE_WORD));
    ...
    state.cycles += 8 + eaCycles(mode, reg, SIZE_WORD);
}
