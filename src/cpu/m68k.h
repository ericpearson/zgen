...
public:
    ...
    // 68K bus refresh: DRAM refresh steals 2 master clocks every 128.
    // Tracked against masterCycles_ to include VDP inter-instruction time,
    // matching GPGX's m68k.cycles counter.
    u64 refreshNextMclk_ = 0;  // next refresh point in master clocks
    bool refreshEnabled_ = false; // enabled by genesis.cpp after system init

    // Called by genesis.cpp after clockM68K to keep master clock counter
    // in sync with VDP time advancement between instructions.
    void advanceMasterClocks(int m68kCycles) { masterCycles_ += m68kCycles * 7; }
