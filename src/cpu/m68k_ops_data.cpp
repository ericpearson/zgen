...
void M68K::op_move() {
    ...
    // 1 extra cycle for byte-sized MOVE between two postincrement or
    // predecrement operands. This matches observed hardware
    // behavior where the bus needs an extra cycle for back-to-back byte
    // transfers with address register updates.
    if (size == SIZE_BYTE && srcMode >= 2 && srcMode <= 4 && dstMode >= 2 && dstMode <= 4) {
        state.cycles += 1;
    }
}
