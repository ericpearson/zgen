...
AppUI::Action AppUI::draw() {
    ...
    if (visible_) {
        ...
        // Gamepad B / Escape: close dialogs or the UI itself.
        // Only act when no popup (submenu/combo dropdown) is open,
        // since ImGui's own nav-cancel handles closing those.
        if (!ImGui::IsPopupOpen((const char*)nullptr, ImGuiPopupFlags_AnyPopup)
            && ImGui::IsKeyPressed(ImGuiKey_GamepadFaceRight, false)) {
            bool anyDialog = showFileBrowser_ || showSaveStates_ || showCheatManager_ ||
                             showVideoSettings_ || showAudioSettings_ || showEmuSettings_ ||
                             showInputSettings_ || showAbout_ || showControls_;
            if (anyDialog) {
                // Close all dialogs and re-focus the menu bar
                showFileBrowser_ = showSaveStates_ = showCheatManager_ = false;
                showVideoSettings_ = showAudioSettings_ = showEmuSettings_ = false;
                showInputSettings_ = showAbout_ = showControls_ = false;
                justBecameVisible_ = true; // re-focus menu bar next frame
            } else {
                // No dialog open — close the entire UI
                visible_ = false;
                Action a;
                a.type = ActionType::CloseMenu;
                pushAction(a);
            }
        }
    }
...
void AppUI::drawMenuBar() {
    if (ImGui::BeginMainMenuBar()) {
        if (justBecameVisible_) {
            ...
            // Focus the menu bar and activate its nav layer so gamepad
            // D-pad immediately highlights "File" without needing extra input
