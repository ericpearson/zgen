...
class AppUI {
public:
    // Navigation panels in the sidebar
    enum class Panel {
        Resume,
        ROM,
        SaveStates,
    ...
    // Sub-tabs within the Settings panel
    ...
    void setCRTShader(bool v) { crtShader_ = v; }
    float crtCurvature() const { return crtCurvature_; }
    float crtScanline() const { return crtScanline_; }
    float crtBloom() const { return crtBloom_; }
    float crtVignette() const { return crtVignette_; }
    int crtMaskType() const { return crtMaskType_; }
    ...
    void openSaveStates() { activePanel_ = Panel::SaveStates; visible_ = true; }
    void openFileBrowser() { activePanel_ = Panel::ROM; visible_ = true; refreshDirectory(); }
    ...
    void setCurrentRomName(const std::string& name) { currentRomName_ = name; }
...
private:
    ...
    // Active panel and settings tab
    Panel activePanel_ = Panel::Resume;
    SettingsTab settingsTab_ = SettingsTab::Video;

    // Animation state
    float overlayAnimT_ = 0.0f;
    float sidebarTargetY_ = 0.0f;
    float sidebarCurrentY_ = 0.0f;
    ...
    bool crtShader_ = false;
    float crtCurvature_ = 0.20f;
    float crtScanline_ = 0.5f;
    float crtBloom_ = 0.15f;
    float crtVignette_ = 0.5f;
    int crtMaskType_ = 0;  // 0=aperture grille, 1=slot mask, 2=none
    ...
    // Drawing: overlay framework
    void drawOverlay();
    void drawSidebar(float sidebarWidth);
    void drawSidebarIcon(Panel panel, ImVec2 pos, float size);
    void drawContentPanel();

    // Drawing: content panels
    void drawResumePanel();
    void drawROMPanel();
    void drawSaveStatesPanel();
    void drawSettingsPanel();
    void drawCheatsPanel();
    void drawControlsPanel();
    void drawAboutPanel();

    // Drawing: settings sub-tabs
    void drawVideoTab();
    void drawAudioTab();
    void drawInputTab();
    void drawSystemTab();

    // Drawing: input helpers
