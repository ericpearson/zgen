...
static void testDACOverride() {
    ...
    // DAC=0xFF → (255-128)<<6 = 8128 → 9-bit truncation → <<5 = 8128
    // Ladder effect: 6 channels × +128 (all non-negative) = +768
    // Total: 8128 + 768 = 8896
    const YMSample sample = ym.tick();
    check(sample.left == 8896, "DAC overrides FM output on channel 6 left");
    check(sample.right == 8896, "DAC overrides FM output on channel 6 right");
...
static void testPanning() {
    ...
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0xFF); // Positive DAC value

    // Ladder DC offset: 6 channels × +128 (all non-negative) = 768 baseline
    // DAC=0xFF sample = 8128 in 14-bit

    // Left only: left gets signal+ladder, right gets ladder-only DC
    YM2612Test::channel(ym, 5).pan = 0x80;
    YMSample s = ym.tick();
    checkf(s.left == 8896, "L-only: left has signal+ladder: %d (expected 8896)", s.left);
    checkf(s.right == 768, "L-only: right has ladder DC only: %d (expected 768)", s.right);

    // Right only: right gets signal+ladder, left gets ladder-only DC
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0xFF);
    YM2612Test::channel(ym, 5).pan = 0x40;
    s = ym.tick();
    checkf(s.left == 768, "R-only: left has ladder DC only: %d (expected 768)", s.left);
    checkf(s.right == 8896, "R-only: right has signal+ladder: %d (expected 8896)", s.right);

    // Both: left equals right
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0xFF);
    YM2612Test::channel(ym, 5).pan = 0xC0;
    ...
    checkf(s.left == 8896, "LR: both channels: %d (expected 8896)", s.left);

    // Muted: both channels have ladder-only DC (signal not panned)
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0xFF);
    YM2612Test::channel(ym, 5).pan = 0x00;
    s = ym.tick();
    checkf(s.left == 768, "muted: left has ladder DC only: %d (expected 768)", s.left);
    checkf(s.right == 768, "muted: right has ladder DC only: %d (expected 768)", s.right);
...
static void testDACConversion() {
    ...
    // Ladder DC baseline: 6 channels × +128 = 768 when all non-negative

    // DAC=0x80 (128) → (128-128)<<6 = 0 → sample=0, ladder offset only
    YM2612Test::setDacData(ym, 0x80);
    YMSample s = ym.tick();
    checkf(s.left == 768, "DAC 0x80 → ladder DC only: %d (expected 768)", s.left);

    // DAC=0x00 → (0-128)<<6 = -8192 → >>5 = -256 → <<5 = -8192
    // Ch5 ladder: negative, panned → -96. Other 5 ch ladder: +640. Total: -8192 - 96 + 640 = -7648
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    ...
    checkf(s.left == -7648, "DAC 0x00 → %d (expected -7648)", s.left);

    // DAC=0xFF → (255-128)<<6 = 8128 → >>5 = 254 → <<5 = 8128
    // All 6 channels non-negative → ladder +768. Total: 8128 + 768 = 8896
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0xFF);
    YM2612Test::channel(ym, 5).pan = 0xC0;
    ...
    checkf(s.left == 8896, "DAC 0xFF → %d (expected 8896)", s.left);
...
static void testLadderEffect() {
    ...
    // The ladder effect adds ±4 per channel (in 9-bit = ±128 in 14-bit) at
    // the stereo mixing stage. Non-negative channels get +4, negative get -4
    // (or -3 if the channel is panned off on that side).
    // Baseline: 5 silent channels (0-4) each add +128 = 640

    YM2612 ym;
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::channel(ym, 5).pan = 0xC0;

    // Positive: DAC=0x82 → (130-128)<<6 = 128 → >>5 = 4 → <<5 = 128
    // All 6 channels non-negative → ladder +768. Total: 128 + 768 = 896
    YM2612Test::setDacData(ym, 0x82);
    YMSample s = ym.tick();
    checkf(s.left == 896, "DAC 0x82: positive ladder +4/ch: %d (expected 896)", s.left);

    // Negative: DAC=0x7E → (126-128)<<6 = -128 → >>5 = -4 → <<5 = -128
    // Ch5 ladder: -96 (panned → -3). Other 5 ch: +640. Total: -128 - 96 + 640 = 416
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0x7E);
    YM2612Test::channel(ym, 5).pan = 0xC0;
    s = ym.tick();
    checkf(s.left == 416, "DAC 0x7E: negative ladder -3/ch (panned): %d (expected 416)", s.left);

    // Verify asymmetry: unpanned negative channel gets -4 instead of -3
    // Matches GPGX: (4 - (pan & 1)) — pan=0 → offset=4, pan=1 → offset=3
    ym.reset();
    YM2612Test::dacEnabled(ym) = true;
    YM2612Test::setDacData(ym, 0x7E);
    YM2612Test::channel(ym, 5).pan = 0x00; // unpanned
    s = ym.tick();
    // Ch5: sample=-128, not added (unpanned). Ladder: -128 (unpanned → -4<<5)
    // Other 5 ch: +640. Total: -128 + 640 = 512
    checkf(s.left == 512, "unpanned negative: ladder -4/ch: %d (expected 512)", s.left);
...
static void testAlgorithmLevels() {
    ...
    // Both clip to ±8191 per-channel (9-bit DAC range), so at max volume
    // they peak at the same level. Verify both produce significant output.
    checkf(algo7Max >= algo0Max,
           "algo7 peak (%d) >= algo0 peak (%d) — both clip at DAC max",
           algo7Max, algo0Max);
