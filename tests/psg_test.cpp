...
// =====================================================================
// Test 21: Noise output polarity (LFSR bit 0)
// =====================================================================
static void testNoiseOutputPolarity() {
    ...
    // LFSR = 0x8001: bit 0 = 1, so (lfsr & 1) = true = positive contribution
    PSGTest::lfsr(psg) = 0x8001;
    PSGTest::tickOnce(psg);
    s16 sample1 = psg.getSample();
    check(sample1 > 0, "LFSR bit0=1: positive output");

    // LFSR = 0x8000: bit 0 = 0, so (lfsr & 1) = false = negative contribution
    PSGTest::lfsr(psg) = 0x8000;
    PSGTest::tickOnce(psg);
    s16 sample2 = psg.getSample();
    check(sample2 < 0, "LFSR bit0=0: negative output");

    // Magnitudes should be equal (same volume)
    check(sample1 == -sample2, "Positive and negative magnitudes are equal");
...
static void testGetSampleBoundary() {
    ...
    // All negative: -32764 (fits in s16)
    for (int i = 0; i < 3; i++) {
        PSGTest::tone(psg, i).output = -1;
        PSGTest::tone(psg, i).counter = 0x3FF;
    }
    PSGTest::lfsr(psg) = 0x8000; // bit 0 = 0 -> negative

    PSGTest::tickOnce(psg);
    s16 maxNeg = psg.getSample();
    check(maxNeg == -32764, "All max negative = -32764 (no clamp needed)");
...
static void testGetSampleRepeatability() {
    ...
    // Change output polarity, verify fallback updates
    PSGTest::tone(psg, 0).output = -1;
    s16 s4 = psg.getSample();
    check(s4 == -8191, "Fallback reflects output change (-8191)");
